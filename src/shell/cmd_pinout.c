/* cmd_pinout.c - GPIO Pin Visualizer (multi-board) */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
#define NUM_GPIO BOARD_GPIO_COUNT
...
/* Board-specific pinout tables */
#if defined(ADAFRUIT_FEATHER_RP2350)
#include "board/pinout_feather_rp2350.h"
#else
#include "board/pinout_pico.h"
#endif
...
static void print_board(void) {
    ...
    printf("%s", ANSI_BOLD);
    for (int i = 0; i < PINOUT_HEADER_LINES; i++) {
        printf("%s\r\n", pinout_header[i]);
    }
    printf("%s", ANSI_RESET);

    /* Print each row of pins */
    for (int row = 0; row < PINOUT_NUM_ROWS; row++) {
...
static void print_gpio_detail(int gpio) {
    ...
    for (int i = 0; i < PINOUT_NUM_ROWS; i++) {
...
static void cmd_pinout_usage(void) {
    printf("GPIO Pin Visualizer - %s pinout\r\n", BOARD_NAME);
    printf("Usage:\r\n");
