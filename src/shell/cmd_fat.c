 ...
 *
 * Storage: flash partition (FLASH_FAT_PARTITION_OFFSET).
 * No RAM backend — reads/writes go directly to/from flash via
 * flash_fat_read_sector / flash_fat_write_sector.
 * Only the in-memory FAT table (cluster map) lives in RAM.
 */
...
static void fat_setup_flash_backend(void) {
    fat_set_backend(&g_fat_vol, NULL,
                    flash_fat_read_sector,
                    flash_fat_write_sector);
}
...
int cmd_fat(int argc, char *argv[]) {
    if (argc < 2) {
        uint32_t max_sec = flash_fat_get_total_sectors();
        printf("FAT12/FAT16 filesystem (flash-backed)\r\n\r\n");
        printf("Usage: fat <command> [args...]\r\n\r\n");
        printf("  fat init <12|16> [sectors]   Format flash partition (max %lu)\r\n",
               (unsigned long)max_sec);
        printf("  fat mount                    Mount existing volume from flash\r\n");
        printf("  fat unmount                  Unmount volume\r\n");
        printf("  fat erase                    Erase entire FAT partition\r\n");
    ...
    if (strcmp(argv[1], "init") == 0) {
        ...
        /* Initialize flash FAT backend if needed */
        flash_fat_init();

        uint32_t max_sectors = flash_fat_get_total_sectors();
        uint32_t sectors = max_sectors;
        if (argc >= 4) {
            sectors = (uint32_t)atoi(argv[3]);
            if (sectors < 16 || sectors > max_sectors) {
                printf("Sectors must be 16-%lu\r\n", (unsigned long)max_sectors);
                return 1;
            }
        }
        ...
        printf("Erasing FAT partition (%luKB)...\r\n",
               (unsigned long)(sectors * 512 / 1024));
        flash_fat_erase_all();

        fat_setup_flash_backend();
        ...
        printf("Formatted %s on flash: %lu sectors, %lu bytes total, %lu bytes free\r\n",
    ...
    if (strcmp(argv[1], "mount") == 0) {
        ...
        if (r != FAT_OK) {
            printf("No valid FAT volume on flash. Use 'fat init' first.\r\n");
            return 1;
        }

        g_fat_ready = true;
        printf("Mounted %s from flash (%luKB partition)\r\n",
               fat_type_str(g_fat_vol.type),
               (unsigned long)(FLASH_FAT_PARTITION_SIZE / 1024));
        return 0;
    }
    ...
    if (strcmp(argv[1], "erase") == 0) {
        ...
        flash_fat_init();
        printf("Erasing FAT flash partition...\r\n");
        flash_fat_erase_all();
        printf("FAT partition erased (%luKB)\r\n",
               (unsigned long)(FLASH_FAT_PARTITION_SIZE / 1024));
        return 0;
    }
    ...
    if (strcmp(argv[1], "info") == 0) {
        ...
        printf("  Storage:        flash (0x%06X - 0x%06X)\r\n",
               FLASH_FAT_PARTITION_OFFSET,
               FLASH_FAT_PARTITION_OFFSET + FLASH_FAT_PARTITION_SIZE);
        printf("  Type:           %s\r\n", fat_type_str(g_fat_vol.type));
